...
이 실습의 모든 작업은 Azure Portal (PowerShell Cloud Shell 세션 포함)에서 수행됩니다 (실습 1 작업 3 제외). 원격 데스크톱 세션에서 Azure VM까지 수행되는 단계가 포함됩니다
...
Adatum Corporation은 Azure VM 호스팅 웹 워크로드를 구현하고 Azure Load Balancer의 NAT(Network Address Translation) 기능을 활용하여 고가용성 방식으로 자회사 Contoso Corporation에 대한 관리를 용이하게 하려고 합니다.
...
1. Azure Resource Manager 템플릿을 사용하여 첫 번째 Azure 지역에서 가용성 집합에 설치된 IIS(웹 서버) 역할을 사용하여 Windows Server 2016 데이터 센터를 실행하는 관리 Azure VM배포
...
#### 작업 1: Azure Resource Manager 템플릿을 사용하여 첫 번째 Azure 지역에서 가용성 집합에 설치된 IIS(웹 서버) 역할을 사용하여 Windows Server 2016 데이터 센터를 실행하는 관리 Azure VM배포
...
1. **사용자 지정 배포** 블레이드에서 다음 설정을 사용하고 템플릿 배포를 시작합니다.
    ...
    - 위치: 이전 작업에서 선택한 것과 다른 Azure 지역의 이름과 Azure VM을 준비 할 수있는 위치
...
1. **리소스 만들기** 블레이드를 통해 Azure Marketplace에서 **부하 분산 장치**를 검색하십시오.

1. 검색 결과 목록을 사용하여부하 분산 장치 **만들기** 블레이드로 이동하십시오.
...
1. **부하 분산 장치 만들기** 블레이드에서 다음 설정으로 새 Azure load balancer 를 만듭니다.
    ...
    - 공용 IP 주소: **az1010301w-lb-pip** 라는 새 공용 IP 주소
    ...
    - 위치: 이전 연습의 첫 번째 작업에 Azure VM을 배포한 Azure 지역의 이름입니다

1. Azure Portal에서 새로 배포된 Azure Load Balancer 장치인 **az1010301w-lb**의 블레이드로 이동합니다.
...
1. Azure Portal에서 새로 배포된 Azure Load Balancer 장치인 **az1010302w-lb**의 블레이드로 이동합니다.
...
1. Azure Portal에서 Azure load balancer **az1010301w-lb**의 블레이드로 이동합니다.
...
1. Azure Portal에서 Azure Load Balancer **az1010302w-lb**의 블레이드로 이동합니다.
...
> **결과**: 이 연습을 완료한 후 두 지역에서 Azure Load Balancer의 부하 분산 규칙 및 NAT 규칙을 구현하고 확인했습니다.
...
1. Azure Portal에서 **az1010301w-lb-pip**라는 첫 번째 지역에서 Azure Load Balancer 와 연결된 공용 IP 주소 리소스의 블레이드로 이동합니다. 
...
1. **az1010302w-lb-pip**라는 두 번째 지역에서 Azure Load Balancer 와 연결된 공용 IP 주소 리소스의 블레이드로 이동합니다. 
...
1. **리소스 만들기** 블레이드를 통해 Azure Marketplace에서 **Traffic Manager 프로필**을 검색하십시오.
...
1. **끝점** 블레이드에서 두 끝점에 대한 **모니터링 상태** 열의 항목을 검사합니다. 다음 작업을 진행하기 전에 둘 다 **온라인**으로 나열될 때까지 기다립니다.
