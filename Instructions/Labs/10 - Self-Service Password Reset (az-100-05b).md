...
1. **AdatumLab100-5b - 개요** 블레이드에서 **라이선스 - 개요** 블레이드로 이동합니다.

1. **라이선스 - 개요** 블레이드에서 **제품**블레이드로 이동합니다. 
...
1. **그룹에서 - 모든 그룹** 블레이드에서 **그룹** 블레이드로 이동하고 다음 설정을 사용하여 새 그룹을 만듭니다.
...
1. **암호 재설정 - 속성** 블레이드에서 **암호 재설정으로 이동 - Auhentication 방법**블레이드를 탐색하고 다음 설정을 구성합니다.
    ...
    - 사용자가 사용할 수 있는 방법: 
        ...
        - 보안 질문을 선택 **미리 정의됨**을 선택하고 5 개 미리 정의된 보안 질문의 조합을 추가
...
1. **추가 정보 필요** 메시지가 표시되면 **계정에 대한 액세스 권한 유지** 페이지로 진행하십시오.
...
1. 새 브라우저 창에서 Azure Portal로 이동 한 다음 **계정 선택** 페이지에서 **aaduser1**사용자 계정 이름을 입력합니다. 
...
1. 새로 재설정된 암호를 사용하여 Azure Portal에 로그인할 수 있는지 확인합니다. 
...
1. **Microsoft OneDrive - 개요** 블레이드에서 **Microsoft OneDrive - 시작하기** 블레이드로 이동합니다. 
...
1. 응용 프로그램 액세스 패널에서 로그아웃하고 Microsoft Edge 창을 닫습니다.
   ...
   > **참고**: Microsoft Edge를 다시 시작하고, Azure Portal로 검색하고, 이 랩에서 사용 하 던 Azure 구독에서 소유자 역할을 하는 Microsoft 계정을 사용 하 여 로그인 하고 **디렉터리 + 구독 필터**를 사용하여 기본 Azure로 전환 해야 합니다.이 랩을 완료하면 AD 테넌트입니다.
