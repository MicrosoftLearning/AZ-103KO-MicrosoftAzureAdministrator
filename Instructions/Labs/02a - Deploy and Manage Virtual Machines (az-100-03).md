...
이 실습의 모든 작업은 원격 데스크톱 세션에서 Azure VM에 수행된 단계를 포함하는 연습 2 작업 2 및 연습 2 작업 3를 제외하고 Azure 포털(PowerShell Cloud Shell 세션 포함)에서 수행됩니다.
...
1. 실습 가상 머신에서 Microsoft Edge를 시작하고 [**http://portal.azure.com**](http://portal.azure.com) 에서 Azure 포털을 찾은 다음, 이 실습에서 사용하려는 Azure 구독에서 소유자 역할이 있는 Microsoft 계정을 사용하여 로그인합니다.

1. Azure 포털에서 **리소스 만들기** 블레이드로 이동합니다.

1. **리소스 만들기** 블레이드에서 **Windows Server** 를 Azure 마켓플레이스에서 검색합니다. 검색 결과 목록에서 **Windows Server** 를 선택합니다.

1. Windows Server 페이지에서, 드롭 다운 메뉴를 사용하여 **[smalldisk] Windows Server 2016 Datacenter** 를 선택하고, **만들기**를 클릭합니다.

1. **가상 머신 만들기** 블레이드에서 다음 설정을 사용하여 가상 머신를 배포합니다.

    - 구독: 이 실습에서 사용 중인 구독의 이름

    - 리소스 그룹: 새 리소스 그룹 **az1000301-RG** 의 이름

    - 가상 머신 이름: **az1000301-vm0**

    - 지역: 실습 위치에 가장 가깝고 Azure VM을 프로비저닝할 수 있는 Azure 지역의 이름입니다.

    - 가용성 옵션: **가용성 집합**

    - 가용성 집합: **새로 만들기**를 클릭하고, **2** 개의 장애 도메인과 **5** 개의 업데이트 도메인으로, 그리고 이름은 **az1000301-avset0**로 설정하여 **확인**을 클릭합니다.
...
1. 네트워킹 탭에서, 가상 네트워느에 있는 **새로 만들기**를 클릭합니다. 디폴트로 설정된 가상 네트워크 이름을 사용하고 다음을 지정합니다:
...
1. 다른 부분은 디폴트 값을 그대로 두고, **검토 + 만들기**를 클릭합니다.

1. **만들기**를 클릭합니다.

   > **참고**: 이 작업에서 만들어진 네트워크 보안 그룹을 이 실습의 다음 연습에 있는 작업에서 구성할 것입니다.
...
1. 클라우드 셸 창에서 다음 명령을 실행합니다:
   ...
   > **참고**: 이 실습의 두 번째 연습에서는 이 작업에서 만든 네트워크 보안 그룹을 구성할 것입니다.
...
1. 클라우드 셸 창에서 다음 명령을 실행합니다:
   ...
   > **참고**: 이러한 명령은 새 Azure VM을 프로비저닝하는 데 사용할 Azure Marketplace 이미지의 속성을 지정하는 변수값을 설정합니다.
...
1. 클라우드 셸 창에서 다음 명령을 실행합니다:
   ...
   > **참고**: 이들 명령은 VM 크기, VM의 가용성 집합, 네트워크 인터페이스, 컴퓨터 이름, 로컬 관리자 자격 증명, 원본 이미지, 운영 체제 디스크, 부팅 진단 설정 등, 새 Azure VM을 프로비저닝하는 데 사용되는 Azure VM 구성 개체의 속성을 설정합니다.
...
#### 작업 3: Azure 리소스 관리자 템플릿을 사용하여 Linux를 실행하는 Azure VM 두 대를 가용성 집합에 배포합니다
...
1. RDP 세션 내에서 **az1000301-vm0,** **Command Prompt** 를 시작합니다.
...
1. **az1000301-vm0** 에 RDP 세션 내에서,인터넷 익스플로러를 시작하고 [**https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html**](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) 에서 **putty.exe**를 다운로드합니다.
...
1. 클라우드 셸 창에서 &lt;custom-label&gt; 를 고유한 임의의 문자열로 대체하여 다음 명령을 실행합니다.
...
1. 명령이 **True**로 반환되는지 확인합니다. 그렇지 않은 경우 명령에서 **True** 를 반환할 때까지, &lt;custom-label&gt; 를 다른 값으로 바꾸고 동일한 명령을 다시 실행합니다.

1. 성공적인 결과를 가져오는 &lt;custom-label&gt; 값을 기록하십시오. 다음 작업에 필요합니다
...
1. **리소스 만들기** 블레이드 내 Azure 마켓플레이스에서 **가상 시스템 크기 집합** 을 검색합니다.

1. 검색 결과 목록을 사용하여 **가상 머신 크기 집합** 블레이드에서 만들기로 이동합니다.

1. **가상 머신 크기 집합 블레이드 만들기** 를 사용하여 다음 설정으로 가상 시스템 스케일 집합을 배포합니다.
...
1. **az1000303vms0 - 확장** 블레이드에서 다음설정으로 **PowerShell 원하는 상태 구성 확장** 을 추가합니다.
   ...
   > **참고**: DSC 구성 모듈은 **Labfiles\\AZ100\\Mod03\az-100-03_install_iis_vmss.zip**에서 업로드 할 수 있습니다. 이 모듈에는 IIS(웹 서버) 역할을 설치하는 DSC 구성 스크립트가 포함되어 있습니다.
...
1. 브라우저에 기본 IIS 홈페이지가 표시되는지 확인합니다. 
...
1. 포털 상담에서, 클라우드 셀 창을 열기 위해 **Cloud Shell** 아이콘을 클릭합니다.

1. 클라우드 셀 인터페이스에서, **Bash**를 선택합니다.

1. **Cloud Shell** 명령 프롬프트에서, 다음 명령어를 입력하고 **Enter**를 입력하여 이 실습에서 생성했던 모든 리소스 그룹 목록을 살펴봅니다:

   ```sh
   az group list --query "[?starts_with(name,'az1000')].name" --output tsv
   ```

1. 출력된 결과가 이번 실습에서 생성한 리소스 그룹만을 포함하는지를 확인합니다. 해당 그룹들은 다음 작업에서 삭제될 예정입니다.

#### 작업 2: 리소스 그룹 삭제

1. **Cloud Shell** 명령 프롬프트에서, 다음 명령어를 입력하고 **Enter**를 입력하여 이번 실습에서 생성했던 리소스 그룹을 삭제합니다
...
> **결과**: 이 연습을 통해, 이번 실습에서 사용했던 리소스를 제거하였습니다