...
-  Azure 사이트 복구를 사용하여 Azure 지역 간의 Azure VM 복제 구성
...
1. **Recovery Services 자격 증명 모음** 블레이드를 사용하여 다음 설정을 사용하여 Site Recovery 자격 증명 모음을 만듭니다.
    ...
    - 위치: 이 연습의 이전 작업에서 Azure VM을 배포한 지역과 다른 구독에서 사용할 수 있는 Azure 지역의 이름입니다

> **결과**: 이 연습을 완료한 후 Azure Resource Manager 템플릿을 사용하여 Azure VM 배포를 시작하고 Azure VM 디스크 파일의 콘텐츠를 복제하는 데 사용할 Azure Site Recovery 자격 증명 모음을 만들었습니다. 
...
1. Azure Portal에서 새로 제공된 Azure Recovery Services 볼트 **vaultaz1010102**의 블레이드로 이동합니다.
...
1. **vaultaz1010102** 블레이드에서 다음 복제 설정을 구성합니다.
    ...
    - 대상 위치: 구독에서 사용할 수 있고 이전 작업에서 Azure VM을 배포한 지역과 다른 Azure 지역의 이름입니다. 가능하면 Azure 사이트 복구 자격 증명 모음을 배포한 것과 동일한 Azure 지역을 사용합니다.
